...
We can use STL Map to solve this problem. For every interval, we update the map value of T1[i] by 1 and update map value of T2[i] by -1. Initially map values 
will be 0. This part can be done in O(nlogn). Now lets take a variable X which has been initialised to 0. We have to iterate all the values of map one by one. 
For every value of map, X gets added by the value which the map holds for that particular time. Our answer will be the maximum value of X at any time while 
