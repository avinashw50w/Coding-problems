...
/*Subtask 1
Subtask 1 can be solved by directly simulating the given problem. In other words, we can directly append the number coming in at either ends and check the 
sum for each possible arrangement. There can at maximum be 2N2N different sequences. Therefore, the time complexity is O(2N)O(2N) per test case. This is 
sufficient for this subtask.

Subtask 2
Let us start by thinking of simpler sequences in which observing patterns could be easier. A trick is to take something like 1, 1, 1, ...., 1, 5 as the sequence. 
And then the 5 can be shuffled around to different positions to observe how many times a position is taken into account.

Nevertheless, we are going to take a more mathematical approach in this editorial. Let's see what happens when kthkth number, i.e., A[k]A[k] appears in the 
stream. It can be appended to either of the two ends of the already existing sequence. But how many already existing sequence are there? Clearly, 2(k−1)2(k−1). 
Let us say for now that A[k]A[k] is appended to the right of already existing sequence. Now, consider some pthpth number A[p]A[p] coming after A[k]A[k]. 
How many sequences exist such that the A[p]A[p] will be multiplied by A[k]A[k]? For A[p]A[p] to be multiplied by A[k]A[k], all numbers coming in between 
these two must not go to the side that A[k]A[k] is on, i.e., they should be put on the left in all the 2(k−1)2(k−1) sequences where A[k]A[k] has been 
appended on the right. If this happens, then when A[p]A[p] comes, it will be multiplied by A[k]A[k] when placed on the right of it. The (p+1)th(p+1)th up 
till NthNth numbers can be arranged in any order after that. So how many sequences in total will have the product of A[k]A[k] and A[p]A[p]? Clearly, 
2(k−1)∗2(N−p)2(k−1)∗2(N−p). Thus, total value that gets added to the answer is (A[k]∗2(k−1))∗(A[p]∗2(N−p))(A[k]∗2(k−1))∗(A[p]∗2(N−p)).
...
return 2*ans
This algorithm runs in O(N2)O(N2).

Subtask 3
The algorithm stated in subtask 2 can be made O(N)O(N) by precalculating the suffix sum of the possible_future_sequencespossible_future_sequences array. 
